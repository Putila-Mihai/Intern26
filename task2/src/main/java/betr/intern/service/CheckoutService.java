...
public class CheckoutService {
  ...
  private List<OrderItem> processCartItems(List<CartItem> cartItems) {
    ...
    for (CartItem cartItem : cartItems) {
      Stock stock =
          inventoryRepository
              .findByItem(cartItem.getItem())
              .orElseThrow(
                  () ->
                      new IllegalArgumentException(
                          "Produsul "
                              + cartItem.getItem().getName()
                              + " nu mai exista in magazin!"));

      if (!stock.hasEnoughStock(cartItem.getQuantity())) {
        throw new OutOfStockException(
            "Checkout esuat: Stoc insuficient pentru produsul: "
                + cartItem.getItem().getName()
                + ". Disponibil: "
                + stock.getQuantity());
      }
      ...
      OrderItem orderItem =
          new OrderItem(cartItem.getItem(), cartItem.getQuantity(), cartItem.getItem().getPrice());
