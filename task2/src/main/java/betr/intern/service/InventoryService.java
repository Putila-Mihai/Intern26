...
public class InventoryService {
  ...
  public Item registerItem(
      Long id, String name, String details, double price, Category category, int initialStock) {
  ...
  public void addStock(Item item, int amount) {
    Stock stock =
        inventoryRepository
            .findByItem(item)
            .orElseThrow(
                () -> new IllegalArgumentException("Produsul nu a fost gasit in inventar."));
  ...
  public void printItemsByCategory(Category category) {
    ...
    List<Stock> filtered =
        inventoryRepository.findAll().stream()
            .filter(
                stock ->
                    stock.getItem().getCategory() != null
                        && stock.getItem().getCategory().equals(category))
